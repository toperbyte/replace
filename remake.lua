...
element.add_colorpicker = function(cp_cfg)
    ...
    local function update_cp()
        ...
        if cp_cfg.callback then
            cp_cfg.callback(color)
        end
    end
...
return element
    ...
    local function get_display_text(v)
        if type(v) == "table" then
            return tostring(v.name or v.text or v[1] or "Unknown Table")
        end
        return tostring(v)
    end
    ...
    local function refresh_list()
        ...
        for _, item in pairs(items) do
            local btn = library.create("TextButton", { 
                ...
                Size = UDim2.new(1, 0, 0, 20), 
                BackgroundTransparency = 1, 
                Text = "  " .. get_display_text(item), 
                FontFace = library.font, 
                TextSize = 12, 
                TextColor3 = (item == selected_item) and library.theme.Accent or library.theme.Text, 
                TextXAlignment = Enum.TextXAlignment.Left,
                AutoButtonColor = false
            })
    ...
    local element = { 
        ...
        remove = function(v) 
            for i, x in pairs(items) do 
                ...
                if x == v or (type(x) == "table" and type(v) == "table" and (x.name == v.name or x[1] == v[1])) then 
                    table.remove(items, i) 
                    if selected_item == x then selected_item = nil end
                    break 
                end 
            end 
            refresh_list() 
